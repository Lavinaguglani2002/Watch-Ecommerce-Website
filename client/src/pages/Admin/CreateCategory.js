...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(error);
      toast.error("Something went wrong in the form submission");
    }
  };
  ...
  // useEffect to call the getAllCategory function when the component mounts
  useEffect(() => {
    getAllCategory();
  }, []);
          ...
          <div className='col-md-9'>
            ...
            <div>
              <table className="table">
                ...
                <tbody>
                  {categories?.map((c) => (
                    <tr key={c._id}>  {/* Unique key on the parent element */}
                      <td>{c.name}</td>
                      <td>
                        <button className='btn btn-primary ms-2 p-2' onClick={() => {
                          setVisible(true);
            ...
            <Modal onCancel={() => setVisible(false)} footer={null} visible={visible}>
              <CategoryForm value={updatedName} setValue={setUpdateName} handleSubmit={handleUpdated} />
            </Modal>
          </div>
        </div>
      </div>
